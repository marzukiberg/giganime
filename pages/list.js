...
export default function ListAnime(props) {
  ...
  return (
    <>
      <Header title="Daftar Anime - GigaNime" />

      <main>
        <article>
          <ArticleTitle title={"Daftar Anime"} />
          <div className="mb-3 p-3 flex justify-center flex-wrap">
            {lists?.map((item) => {
              return (
                <a
                  href={"#letter-" + item.abjad}
                  key={item.abjad}
                  className="text-white w-8 h-8 bg-blue-400 hover:bg-blue-300 flex items-center justify-center"
                >
                  {item.abjad}
                </a>
              );
...
export async function getServerSideProps() {
  const listRes = await fetch(apiURL + "list");
